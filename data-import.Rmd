...
# What you need to know before hand
  ...
  .big[How to use a function]<br>
  .big[assign values and outputs to objects]
  
---

# What are we learning today?

.big[Importing(and exporting) data from(and to):]
  + commas separated values (csv)<br><br>
  + tab separated values (tsv)<br><br>
  + google sheets<br><br>
  + bigquery<br><br>
...
.big[ __Main functions__: `read_*`, `write_*`]<br><br>

.big[ __Type of files that can be handled__: `csv`, `tsv` and `fwf` ]

.big[ __This packages is loaded when library tidyverse is loaded__]
...
.big[ __Important Arguments for `read_*`__:]

  + `file`: Location of the file. URLs are acceptable. Zip files will be automatically uncompressed.<br><br>
  + `col_names`: TRUE(default,FALSE or character vector of column names.<br><br>
  + `col_types`: One of NULL, a cols() specification, or a string.<br><br>

]

---

# Example for reading in csv

.pull-left[
```{r eval=FALSE}
mtcars <- read_csv("D:/folder1/filename.csv", 
                   col_types = list(
    mpg = col_double(),
    cyl = col_integer(),
    disp = col_double(),
...
.big[ __Important Arguments for `write_*`__:]

  + `x`: A data frame or tibble to write to.<br><br>
  + `file`: Location of the file,along with name, where you want save it.<br><br>
...
# Example for writing a csv


```{r eval=FALSE}

penguins %>% 
  write(file = here::here("Data","filename.csv"))
...
# Demo for reading/writing data from google sheets.

[Link to documentation](https://googlesheets4.tidyverse.org/)
...
# Demo for reading/writing data from bigquery

[Link to documentation](https://bigrquery.r-dbi.org/index.html)
